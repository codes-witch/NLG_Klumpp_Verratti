...
class CocoDataset(data.Dataset):
    ...
    # TODO (my comment) (all comments here are original)
    ...
    def get_tokenized_captions(cls, caption_path, target_path):
        # Load tokenized captions if possible
        if os.path.exists(target_path):
            with open(target_path, 'rb') as f:
                tokens = pickle.load(f)
        else:
            # Nie et al.'s original code builds tokenized captions here, but since we already have them,
            # we do not necessarily need that for reproduction
    ...
    def get_vocabulary(cls, vocab_path, captions_path, tokenized_captions, threshold=1):
        ...
        if os.path.exists(vocab_path):
            ...
            # Nie et al.'s original code builds and saves the vocabulary here, but since we already have it,
            # we do not necessarily need that for reproduction
            raise NotImplementedError("Not implemented in this version")
        return vocab
