...
class GVE(LRCN):
	...
	# return a function that can be used as feat_func in lrcn.forward(), but that makes sure that labels are concatenated to the image features
	# 'labels' are the labels, 'labels_onehot' is a one-hot representation of them
	...
	# what is returned is a function corresponding to 'append_labels' with 'labels_onehot' as the first argument (so it still needs 'image_features' as a second argument)
