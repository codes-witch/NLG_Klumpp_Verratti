...
class CocoDataset(data.Dataset):
    ...
    # NOTE: For COCO, we do not have 'test' here because attributes are not specified in the data (hence the
    ...
    def __init__(self, root, split='train', vocab=None, tokenized_captions=None,
        ...
        # NOTE: cls is needed to use the classmethods defined below
        ...
        # NOTE: the paths are defined depending on the root directory (where all data is contained)
        ...
        # NOTE: get tokenized captions (either by loading or by constructing them)
        ...
        # NOTE: get the vocabulary using (TODO how) the path to the training data (?),
        # which is just the normal path and the normal tokenized captions for training, but
        # has to be specified separately for validation
        if vocab is None:
            if split != 'train':
        ...
        # NOTE: TODO Why do we need that? Why can't we just always use the images?
        if split == 'train':
        ...
        # NOTE: set self.ids_based_on to the chosen base for the image ids
    ...
    """
    # NOTE TODO: Is this used, and if so, where? -> line 84 main_notes throws error without this
    ...
    def build_tokenized_captions(cls, json):
        # NOTE: The PTBTokenizer can be found in /utils/tokenizer/ptbtokenizer.py
    ...
    def get_tokenized_captions(cls, caption_path, target_path):
        ...
        # NOTE: if there is something at target_path, we can get the captions from there,
    ...
    def build_vocab(cls, json, tokenized_captions, threshold):
        # NOTE: The vocabulary contains all words that occur above a certain threshold
    ...
    def get_vocabulary(cls, vocab_path, captions_path, tokenized_captions, threshold=1):
        ...
        # NOTE: Loading is done if there is something in vocab_path, else the classmethod build_vocab (above)
        # is called and the resulting vocabulary is stored under vocab_path
